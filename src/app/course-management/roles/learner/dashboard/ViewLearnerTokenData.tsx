...
import {
  getConnectedTokenAsset,
  hexToString,
  LearnerReferenceUTxO,
  queryIndividualLearnerData,
} from "@andamiojs/core";
...
export default function ViewLearnerTokenData() {
  ...
  const [connectedLearnerToken, setConnectedLearnerToken] = useState<
    string | undefined
  >(undefined);
  const [learnerReferenceUTxO, setLearnerReferenceUTxO] = useState<
    LearnerReferenceUTxO | undefined
  >(undefined);
  ...
  useEffect(() => {
    async function getLearnerDatum() {
      if (connectedLearnerToken) {
        const _datum = await queryIndividualLearnerData(
          wallet,
          andamioConfig,
  ...
  }, [connectedLearnerToken, wallet, connected]);
  ...
  return (
    <div className="grid grid-cols-2 gap-5 w-full">
      <div className="col-span-2 card shadow-xl p-5 bg-secondary text-secondary-content border border-primary">
        ...
        {connected ? (
          <div>
            {connectedLearnerToken == undefined &&
            learnerReferenceUTxO == undefined ? (
              <div>
                <p className="py-3">
                  You do not yet hold Learner Token. Click here to mint one:
                </p>
              ...
              <>
                <div className="pb-3">
                  <div className="">
                    {connectedLearnerToken ? (
                      <>
                        {learnerReferenceUTxO?.data?.learnerCS ==
                        connectedLearnerToken?.substring(0, 56) ? (
                          ...
                          <div className="p-3 bg-error font-bold text-center">
                            Your Learner Token is not valid. Ask for help on
                            Discord.
                          </div>
                        )}
                      </>
                      ...
                      <p className="font-bold">
                        ...
                        {address &&
                        learnerReferenceUTxO &&
                        learnerReferenceUTxO?.data?.rewardAddress.fields[0]
                          .fields[0] == resolvePaymentKeyHash(address)
                  ...
                  <h2 className="text-2xl py-3 mt-3 border-t border-secondary-content">
                    About Your Learner Token:
                  </h2>
                  {/* <pre>{JSON.stringify(learnerReferenceUTxO?.data, null, 2)}</pre> */}
                  <pre>Asset Id: {connectedLearnerToken}</pre>
                  <pre>
                    Policy Id: {connectedLearnerToken?.substring(0, 56)}
                  </pre>
                  <pre>
                    Token Name:{" "}
                    {connectedLearnerToken &&
                      hexToString(connectedLearnerToken?.substring(56))}
                  </pre>
                  <h2 className="text-2xl py-3 mt-3 border-t border-secondary-content">
      ...
      {connected &&
        (connectedLearnerToken || learnerReferenceUTxO?.data?.learnerInfo) && (
          <>
              ...
              <div>
                <div>
                  ...
                  {learnerReferenceUTxO?.data?.learnerInfo ? (
                    <>
                      <p className="py-2">
                        You can add a string, up to 64 characters in length, to
                        your Learner Token. It can be a friendly message, a goal
                        you want to accomplish, a social media handle, or a
                        personal URL. We are excited to learn along with you how
                        to use this optional personal information.
                      </p>
                      <p className="py-2 font-bold">
                        Just remember that whatever personal information you
                        add, it will be public on the blockchain.
                      </p>
                      <div className="bg-primary text-primary-content p-3 mb-3">
                        <h2 className="text-xl font-bold text-info pb-3">
                ...
                {learnerReferenceUTxO && (
                  <BurnLearnerReferenceTokensModal
                    learnerReferenceUTxO={learnerReferenceUTxO}
                  />
                )}
              </div>
            ...
            <div className="card shadow-xl p-5 bg-secondary text-secondary-content border border-primary">
              <div>
                <div>
                  <h2 className="text-2xl text-secondary-content font-mono pb-5">
                    Current Assignment
                  </h2>
                  <>
                    {learnerReferenceUTxO?.data?.currentAssignment &&
            ...
            <div className="card shadow-xl p-5 bg-secondary text-secondary-content border border-primary">
              <div>
                ...
                <div>
                  <h2 className="text-xl py-3 mt-3 border-t border-secondary-content">
                    Completed Modules:
                  </h2>
                  <div className="grid grid-cols-2 lg:grid-cols-3 gap-5">
                    {learnerReferenceUTxO?.data?.completedAssignments.map(
                      (a: string) => <CompletedModule moduleId={a} />
                    )}
                  </div>
                </div>
                <div>
                  <h2 className="text-xl py-3 mt-3 border-t border-secondary-content">
                    Contributor Credentials Earned
                  </h2>
                  <p>Coming Soon!</p>
                </div>
              </div>
